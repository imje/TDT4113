...
# up letters and digits in this dictionary.
    ...
    def __init__(self,sport=True):
        if sport:
            self.serial_port = arduino_connect.basic_connect('/dev/cu.wchusbserial1420')
        self.reset()
    ...
    def process_signal(self,signal):
        if (signal == _dot or signal == _dash): #dot or dash
            self.update_current_symbol(signal)
        elif (signal == _symbol_pause): #new symbol
            self.handle_symbol_end()
        elif (signal == _word_pause): # new word
            self.handle_word_end()
        elif (signal == _reset): #reset
            self.handle_word_end()
            self.current_symbol = ''
            self.current_word = ''
            self.current_message = ''
    ...
    def update_current_symbol(self, signal): #Append the current dot or dash onto the end of current symbol
        if (signal == _dot):
    ...
    def handle_symbol_end(self):
        if self.current_symbol in self._morse_codes:
            self.sym += self._morse_codes[self.current_symbol] #finds the symbol corresponding to the input
        self.update_current_word(self.sym) #adds the symbol to the current word
        print("current symbol is: " + self.current_symbol)
        self.current_symbol = ''
        self.sym= ''


    def update_current_word(self, symbol): #Add the most recently completed symbol onto current word.
        self.current_word += symbol

    def handle_word_end(self):
        self.handle_symbol_end()
        self.current_message += self.current_word + ' '
        print("current word is:" + self.current_word) #prints out the complete word
        print ("current message is: " + self.current_message) #prints out the message
        self.current_word = ''
